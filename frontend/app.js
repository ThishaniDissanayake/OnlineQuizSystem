...
const API_BASE = 'http://localhost:8080'; // We'll add REST API later
...
// Check server connection (for now, simulated)
...
// Simulate student data (replace with actual API call)
function simulateStudentData() {
    // This simulates what you'll get from QuizServer.getConnectedClients()
    const mockStudents = [
        { id: 1, name: 'Alice Johnson', connectedAt: new Date().toISOString() },
        { id: 2, name: 'Bob Smith', connectedAt: new Date().toISOString() },
        { id: 3, name: 'Charlie Brown', connectedAt: new Date().toISOString() }
    ];
    
    // Only update if count changed (simulate real updates)
    if (Math.random() > 0.7 && connectedStudents.length < 5) {
        const newStudent = {
            id: connectedStudents.length + 1,
            name: `Student ${connectedStudents.length + 1}`,
            connectedAt: new Date().toISOString()
        };
        connectedStudents.push(newStudent);
        addLog('System', `${newStudent.name} connected`);
    }
    
    updateStudentsList(connectedStudents.length > 0 ? connectedStudents : mockStudents);
}
...
function startQuiz() {
    ...
    // Send START_QUIZ command to QuizServer
